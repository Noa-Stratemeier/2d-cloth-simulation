...
let controls = [
  ...
  { label: "Cloth Rows", min: 2, max: scene.maxRows, step: 1, target: scene, key: "clothRows", rebuild: true },
  { label: "Cloth Columns", min: 2, max: scene.maxColumns, step: 1, target: scene, key: "clothColumns", rebuild: true },
  { label: "Spacing", min: 5, max: scene.maxSpacing, step: 1, target: scene, key: "spacing", rebuild: true },
...
controls.forEach((control, index) => {
  ...
  userInterface.insertAdjacentHTML(
    "beforeend",
    `
        <div class="ui-slider"> 
...
function updateControlLimits() {
  let clothRows = document.getElementById("ui-10");
  let clothColumns = document.getElementById("ui-11");
  let spacing = document.getElementById("ui-12");
  let startX = document.getElementById("ui-13");
  let startY = document.getElementById("ui-14");

  clothRows.max = scene.maxRows;
  clothColumns.max = scene.maxColumns;
  spacing.max = scene.maxSpacing;
  startX.max = scene.width - scene.clothWidth;
  startY.max = scene.height - scene.clothHeight;
}
